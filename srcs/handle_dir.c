...
void	handle_dir_0(t_token **t, t_big_token **b, t_info **info, int (*itscl)[6])
{
	*t = (*info)->tokens;
	(*itscl)[0] = 0;
	(*itscl)[1] = 0;
	...
	count_cmd_args(b, (*b)->ind_tok_start, (&(*info)->tokens), (*b)->length);
...
int	handle_dir_1(t_token **t, int (*itscl)[6], int step, t_big_token **b)
{
	...
	if (step == 2)
	{
		while ((*t) && (*itscl)[4] && (*t)->token != TOK_SEP)
		{
			(*b)->cmd_args[(*b)->cmd_args_num - (*itscl)[3]] = ft_strjoin_free((*b)->cmd_args[(*b)->cmd_args_num - (*itscl)[3]], (*t)->value, 1);
...
int	handle_dir_2(t_token **t, int (*itscl)[6], int step, t_big_token **b)
{
	if (!step)
	{
		if (((*t)->token == TOK_REDIRECTOR_LEFT || (*t)->token == TOK_REDIRECTOR_RIGHT) && !((*itscl)[0] % 2))
			return (1);
	}
	if (step == 1)
	{
...
int	handle_dir(t_big_token **tmp_b, t_info *info)
{
	...
	while (tmp && itscl[4])
	{
		...
		else if ((tmp->token != TOK_SEP && (itscl[0] % 2) && itscl[2]))
		{
			...
			itscl[5] = handle_dir_2(&tmp, &itscl, 1, tmp_b);
	}
	(*tmp_b)->par = 0;
	return (0);
}
